...
class TextJEPA(nn.Module):
    ...
    def forward(self, context_tokens: torch.Tensor, target_tokens: torch.Tensor, mask: torch.Tensor) -> dict:
        ...
        with torch.no_grad():
            ...
            # Align target representations with the mask
            target_representations = target_output[mask]

        context_output = self.context_encoder(context_embeddings)
        
        # The predictor should operate on the full sequence of context + masked positions
        predicted_representations = self.predictor(context_output)
        predicted_masked = predicted_representations[mask]

        loss = F.mse_loss(predicted_masked, target_representations)

        return {"loss": loss, "predictions": predicted_masked, "targets": target_representations}
